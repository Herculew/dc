...
func main(){
	...
	for rows.Next(){
		...
		//获取单个表所有字段
		cloumn_sql := fmt.Sprintf("select column_name columnName, data_type dataType, column_comment columnComment, column_key columnKey, extra, column_type columnType from information_schema.columns where table_name = '%s' and table_schema = (select database()) order by ordinal_position",table.Name)
		...
		column := Column{}
		for cloumns.Next(){
			err := cloumns.Scan(&column.Columname,&column.Datatype,&column.Columncomment,&column.Columnkey,&column.Extra,&column.Columntype)
			//类型判断
			//开始拼接字符串
			if err != nil {
				fmt.Printf("Scan failed,err:%v", err)
			...
			struct_str += "    "+strToUpper(column.Columname)
			...
			if column.Extra != "auto_increment"{
				struct_str += fmt.Sprintf("`xorm:\"%s\" json:\"%s\"` \n", strings.TrimSuffix(column.Columntype, " unsigned"),column.Columname)
			}else{
				struct_str += fmt.Sprintf("`xorm:\"primary_key autoincr comment('%s') INT(11)\" json:\"%s\"` \n",column.Columncomment,column.Columname)
			}
